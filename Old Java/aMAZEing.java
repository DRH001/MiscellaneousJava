...
public class aMAZEing extends Applet implements Runnable{
	...
	*/
	...
	int frameSkip = 2;//shows every n frames (2 is good for most applications with 2 periodicity)
	int desiredFPS = 200;//maxes out at like 500 ish 
	...
	int tickReset = 500000000;//set to like 1000000000 if you don't want a periodic reset
	boolean deadReset = false;
	...
	int xSize = 100;//usually 600, is how many pixelsize pixels the screen is//keep this odd if using custom stuff for more symmetric results
	...
	boolean mazeFlag = true; //keep this true for a maze generator
	...
	double spawnFactor = 0.795; //usually 0.8 - 1-spawnFactor is the percentage chance each pixel will be true when spawning randomly
	...
	*/
	...
	//rules:
	//*
	int[] liveNumbers = {-1,1,2,3,-1,-1,-1,-1,-1};//must absolutely have 9 digits, cells will not die with this many neighbors - usually 2 and 3
	int[] regenNumberList = {2,-1,-1,-1,-1,-1,-1,-1,-1}; //the number of neighbors which resurrects a dead cell, usually 3 (MUST ABSOLUTELY BE 9 numbers!!!!!!!)
	/* 
	...
	*/
	...
	public void updateBoard(boolean lastTick) {//contains reset/dead detection info, as well as rules
		...
		if(ticks%checkEveryNFrames==0){
			for(int i = 0; i<xSize; i++){
				for(int j = 0; j<ySize; j++){
					oldGame[i][j] = shownGame[i][j];
				}
			}
		}
		...
		if(ticks%checkEveryNFrames==checkEveryNFrames-1){
			System.out.println(ticks);
			outerloop:
			for(int i=0;i<xSize;i++){
				for(int j=0;j<ySize;j++){
					if(oldGame[i][j] == updatedGame[i][j]){
			...
			if((dead==xSize*ySize||(mazeFlag==true && ticks>60))&&!deadReset){
